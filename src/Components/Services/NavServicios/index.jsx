    ...
    <div className="box-border phone:w-full mac:w-4/5   ">
      ...
      <div className="flex flex-row tablet:gap-5 p-3 phone:flex-wrap phone:items-center phone:justify-center phone:gap-2 laptop:flex-nowrap">
        {servicios.map((item, index) => (
          <div
            className={` ${
              phone && card === item.value
                ? 'bg-red-700 text-white'
                : 'bg-white text-black'
            } phone:w-full phone:h-10 tablet:w-1/5 tablet:h-16 flex flex-row justify-center items-center rounded-2xl gap-5  shadow-all cursor-pointer phone:p-2 mac:p-0 phone:hover:text-white tablet:hover:text-black phone:hover:bg-red-700 tablet:hover:bg-transparent duration-300 ease-in-out  `}
            key={index}
